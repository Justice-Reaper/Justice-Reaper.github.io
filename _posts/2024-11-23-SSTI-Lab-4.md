...
Este `laboratorio` es `vulnerable` a un `Server Side Template Inyection`. Para resolver el laboratorio, debemos `identificar` el `motor` de `plantillas` y encontrar un `exploit` documentado el cual podamos usar para `ejecutar comandos`, posteriormente `eliminamos` el archivo `morale.txt` del directorio de `inicio` de `Carlos`
