...
Para `comprobar` que `funciona` nos dirigimos a nuestro `Exploit server` y pegamos el `payload`. `Las etiquetas html y body no son necesarias` para que `funcione` el `ataque CSRF`, en este caso `no las uso para que se vea mejor el payload`
