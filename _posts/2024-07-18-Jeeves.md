...
Pegamos este `payload` y le damos a `execute`
...
En `Backup stuff` hay un hash `NTLMV2` el cual podemos usar para hacer `Pass the hash`, para profundizar en el tema recomiendo este `blog` [https://deephacking.tech/que-es-y-por-que-funciona-pass-the-hash-autenticacion-ntlm/#que-es-pass-the-hash](https://deephacking.tech/que-es-y-por-que-funciona-pass-the-hash-autenticacion-ntlm/#que-es-pass-the-hash)
