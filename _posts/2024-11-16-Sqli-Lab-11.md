...
Este laboratorio contiene una `vulnerabilidad` de `Blind SQL Inyection`. La web utiliza una `cookie` de seguimiento para analíticas y realiza una `consulta SQL` que incluye el valor de la cookie enviada. Los resultados de la `consulta SQL` no se `devuelven` ni se muestran `mensajes` de `error`. Sin embargo, la web incluye un mensaje de `Bienvenido de nuevo` en la pagina si la consulta devuelve alguna fila. La `base` de `datos` contiene una `tabla` diferente llamada `users`, con `columnas` llamadas `username` y `password`. Debemos `explotar` la `Blind SQL Inyectiion` para averiguar la contraseña del usuario `administrador`. Para resolver el laboratorio, hay que `inicia sesión` como el usuario `administrador`
