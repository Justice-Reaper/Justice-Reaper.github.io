...
Nos descargamos `sharphound` [https://github.com/BloodHoundAD/SharpHound/releases/tag/v2.0.0](https://github.com/BloodHoundAD/SharpHound/releases/tag/v2.0.0) y desde el `mismo directorio` donde está `SharpHound.exe` nos `conectamos` usando `evil-winrm` y `subimos` el `archivo` a la `máquina víctima`
