...
6 - Si no encontramos nada, vamos a inspeccionar manualmente los archivos JavaScript que se nos han cargado después de interactuar con todas las funcionalidades del sitio web. Podemos pedir a la IA que nos lo analice para ver si ve algo que nos permita explotar un XSS. Si vemos algún archivo que carge un framework estilo Angular, vamos a poner al inicio del diccionario los payloads específicos para ese framework

7 - Si no hay nada vamos a introducir nosotros un payload simple como este <h1>test</h1>, en todos los puntos en los que podemos introducir datos y posteriormente vamos a mirar el código fuente a ver si podemos escapar nuestro payload y lograr que se ejecute. Para esto también podemos utilizar la IA

8 - Una aclaración respecto a XSS Hunter, si la web utiliza HTTP/1.1 tenemos que urlencodear los payloads. Si lo anterior no ha funcionado, vamos a utilizar la herramienta XSS-Hunter junto con este diccionario [https://raw.githubusercontent.com/coffinxp/loxs/refs/heads/main/payloads/xss.txt](https://raw.githubusercontent.com/coffinxp/loxs/refs/heads/main/payloads/xss.txt). Este diccionario vamos a modificarlo y vamos insertarle al inicio los payloads de todos los laboratorios de XSS. Respecto a XSS-Hunter, debemos de lanzarlo en una petición por GET a la raíz / aunque no haya nada en la URL, por ejemplo, si vemos esto en Bursputie GET / HTTP/2 ahí tenemos que lanzarlo y también debemos hacerlo con peticiones así GET /? HTTP/2. El ? lo añadimos nosotros manualmente y justo después de la ? fuzzeamos con XSS-Hunter

9 - Ahora vamos a hacer unas aclaraciones importantes sobre los puntos anteriores. Lo primero, cuando nos encontremos un formulario es probable que campos como el email o el enlace a una web necesiten cumplir un patrón exacto para considerarse válidos. Tenemso que enviar la petición desde Burpsuite y comprobar si nos sigue pidiendo que cumplamos con cierto patrón. Si no nos pide que cumplamos con el patrón, puede haber un XSS. Hay otros casos en los que puede haber un XSS aunque tengamos que usar un enlace a una web (http://payloadXSS)

Segundo punto, si se envían datos por POST con esta estructura productId=1&storeId=London y en la url de la web vemos que la petición por GET a /product?productId=1 nos devuelve un producto podemos intentar añadir en la url el dato storeid que se ha enviado por post y buscar en la URL resultante un XSS /product?productId=1&storeId=FuzzearXSS. Aquí lanzaríamos primero un análisis del insertion point y luego lanzaríamos XSS-Hunter

Tercer punto, cuando usemos XSS Hunter hay que revisar para ver si las respuestas nos dicen algo como tag blocked o event blocked. Si esto pasa, vamos a mandar una peticion al Intruder con este payload <test>probando</test> y vamos a inyectar un payload en que sustituya test y vamos a fuzzear los tags o events para ver cuales podemos usar. Para fuzzear vamos a usar esta cheatsheet [https://portswigger.net/web-security/cross-site-scripting/cheat-sheet](https://portswigger.net/web-security/cross-site-scripting/cheat-sheet). Una vez sepamos los tags y events que podemos usar vamos a copiarnos los pyaloads de la cheatsheet que cumplan estas condiciones y vamos a detectar si funcionan con XSS Hunter. Si nos sigue bloqueando los events o los tags podemos probar a escribirlos así oNlOaD para intentar bypassear el filtrado
