...
De acuerdo con la `especificación` de `JWS`, solo el parámetro `alg` del `encabezado` es `obligatorio`. Sin embargo, en la práctica, las `cabeceras` del `JWT` a menudo `contienen` otros `parámetros`
