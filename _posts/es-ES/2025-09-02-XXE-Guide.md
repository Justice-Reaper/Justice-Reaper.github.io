...
2. Hacer un `escaneo general` con `Burpsuite` con las extensiones `Active Scan ++` y `Error Message Checks` instaladas. Como `tipo de escaneo` marcaremos `Crawl and audit` y como `configuración de escaneo` usaremos `Deep`
