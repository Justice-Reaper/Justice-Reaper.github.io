...
Un ataque de inyección SQL exitoso puede provocar un acceso no autorizado a datos confidenciales, como:
...
Podemos detectar manualmente una  inyección SQL utilizando un conjunto sistemático de pruebas contra cada punto de entrada de la aplicación. Para ello, normalmente enviaríamos:
...
Como alternativa, podemos encontrar la mayoría de las vulnerabilidades de inyección SQL de forma rápida y fiable utilizando el escáner de Burpsuite
...
Existen muchas vulnerabilidades, ataques y técnicas de inyección SQL que se producen en diferentes situaciones. Algunos ejemplos comunes de inyección SQL incluyen:
...
La aplicación no implementa ninguna defensa contra ataques de inyección SQL. Esto significa que un atacante podría realizar, por ejemplo, el siguiente ataque:
...
En los casos en los que la aplicación responde mostrando los resultados de una consulta SQL, un atacante puede aprovechar una vulnerabilidad de inyección SQL para obtener datos de otras tablas dentro de la base de datos.
...
- SQL injection attack, querying the database type and version on Oracle - [https://justice-reaper.github.io/posts/SQLI-Lab-8/](https://justice-reaper.github.io/posts/SQLI-Lab-8/)

#### Utilizar un ataque de inyección SQL mediante UNION para recuperar datos interesantes
...
Muchos casos de inyección SQL son Blind SQL Injection. Esto significa que la aplicación no devuelve los resultados de la consulta SQL ni los detalles de los posibles errores de la base de datos en sus respuestas

Las blind sql injection todavía pueden explotarse para acceder a datos no autorizados, pero las técnicas utilizadas suelen ser más complejas y difíciles de llevar a cabo

Las siguientes técnicas pueden utilizarse para explotar vulnerabilidades de inyección SQL ciega, dependiendo de la naturaleza de la vulnerabilidad y de la base de datos implicada:
...
Una blind SQL injection ocurre cuando una aplicación es vulnerable a inyección SQL pero sus respuestas HTTP no contienen los resultados de la consulta SQL correspondiente ni los detalles de ningún error de la base de datos
...
- Es posible que podamos provocar mensajes de error que muestren los datos devueltos por la consulta. Esto convierte efectivamente las vulnerabilidades de inyección SQL que de otro modo serían blind en vulnerabilidades visibles
...
En ocasiones, podemos conseguir que la aplicación genere un mensaje de error que contenga algunos de los datos devueltos por la consulta. Esto convierte efectivamente una vulnerabilidad de inyección SQL que de otro modo sería blind, en una vulnerabilidad visible
...
- Visible error-based SQL injection - [https://justice-reaper.github.io/posts/SQLI-Lab-12/](https://justice-reaper.github.io/posts/SQLI-Lab-12/)
...
La inyección SQL de primer orden ocurre cuando la aplicación procesa el input proporcionada por el usuario a través de una petición HTTP e incorpora esa entrada en una consulta SQL de forma insegura
...
Por este motivo, la inyección SQL de segundo orden también se conoce como Stored SQL Injection

La inyección SQL de segundo orden suele producirse en situaciones en las que los desarrolladores son conscientes de las vulnerabilidades de inyección SQL y, por tanto, gestionan de forma segura la introducción inicial de los datos en la base de datos
...
- Blind SQL injection with out-of-band data exfiltration - [https://justice-reaper.github.io/posts/SQLI-Lab-18/](https://justice-reaper.github.io/posts/SQLI-Lab-18/)
...
Sin embargo, también existen muchas diferencias entre las bases de datos habituales. Esto significa que algunas técnicas para detectar y explotar vulnerabilidades de inyección SQL funcionan de manera diferente en distintas plataformas. Por ejemplo:
...
Para explotar una inyección SQL, a menudo es necesario obtener información sobre la base de datos. Esto incluye:
...
- SQL injection attack, listing the database contents on non-Oracle databases - [https://justice-reaper.github.io/posts/SQLI-Lab-6/](https://justice-reaper.github.io/posts/SQLI-Lab-6/)
...
## Ejemplos de tipos de inyecciones SQL
